<template>
  <view class="container">
    <view class="search-bar">
      <u-button type="primary" size="small" shape="circle" icon="plus" v-if="checkPermi(['pp:outputmaster:add'])"
    ...
    <view class="info-item" v-for="(item,index) in dataList" :key="index">
      ...
      <view class="info-line">
        <text class="label-name">生产工单</text>
        <text>{{item.pomOrder}}</text>
      </view>

      <view class="info-line">
        <text class="label-name">工单数量</text>
        <text>{{item.pomOrderQty}}</text>
      </view>

      <view class="info-line">
        <text class="label-name">管理序列号</text>
        <text>{{item.pomOrderSerial}}</text>
      </view>

      <view class="info-line">
        <text class="label-name">生产批次</text>
        <text>{{item.pomMflot}}</text>
      </view>

      <view class="info-line">
        <text class="label-name">机种名</text>
        <text>{{item.pomModelName}}</text>
      </view>

      <view class="info-line">
        <text class="label-name">物料</text>
        <text>{{item.pomMfItem}}</text>
      </view>

      <view class="info-line">
        <text class="label-name">生产日期</text>
        <text>{{item.pomMfDate}}</text>
      </view>

      <view class="info-line">
        <text class="label-name">生产班组</text>
        <text>{{item.pomLineName}}</text>
      </view>

      <view class="info-line">
        <text class="label-name">直接人数</text>
        <text>{{item.pomDirect}}</text>
      </view>

      <view class="info-line">
        <text class="label-name">间接人数</text>
        <text>{{item.pomIndirect}}</text>
      </view>

      <view class="info-line">
        <text class="label-name">标准工时</text>
        <text>{{item.pomStdTime}}</text>
      </view>

      <view class="info-line">
        <text class="label-name">标准产能</text>
        <text>{{item.pomStdOutput}}</text>
      </view>
      ...
      <view class="info-btn-wrap justify-end">
        <view class="tag-item">
          <u-tag text="详情" plain shape="circle" type="info" icon="eye" @click="handleView(item)"
            v-if="checkPermi(['pp:outputmaster:query'])"></u-tag>
        </view>
        <view class="tag-item">
          <u-tag text="编辑" plain shape="circle" icon="edit-pen" @click="handleEdit(item)"
            v-if="checkPermi(['pp:outputmaster:edit'])"></u-tag>
        </view>
        <view class="tag-item">
...
<script>
  ...
  export default {
    ...
    methods: {
      checkPermi,
      getList() {
        listPpOutputMaster(this.queryParams).then(res => {
         if (res.code == 200) {
      ...
      handleEdit(e) {
        this.$tab.navigateTo('./edit?opertype=2&pomId=' + e.id)
      },
      handleView(e) {
        this.$tab.navigateTo('./edit?opertype=3&pomId=' + e.id)
      },
      handleDelete(row) {
        const Ids = row.pomId;
