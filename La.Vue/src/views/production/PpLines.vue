  ...
  <div>
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent>
      ...
      <el-form-item label="班组代码" prop="plLineCode">
        <el-input clearable v-model="queryParams.plLineCode" :placeholder="$t('btn.enter')+'班组代码'" />
      </el-form-item>
      <el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button type="success" :disabled="single" v-hasPermi="['pp:lines:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" :disabled="multiple" v-hasPermi="['pp:lines:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
    ...
    <el-table :data="dataList" v-loading="loading" ref="table" border highlight-current-row @sort-change="sortChange" @selection-change="handleSelectionChange" height="602"
      ...
      <el-table-column :label="$t('btn.operate')" align="center" width="160">
        <template #default="scope">
          <el-button v-hasPermi="['pp:lines:edit']" type="success" icon="edit" :title="$t('btn.edit')" @click="handleUpdate(scope.row)"></el-button>
          <el-button v-hasPermi="['pp:lines:delete']" type="danger" icon="delete" :title="$t('btn.delete')" @click="handleDelete(scope.row)"></el-button>
        </template>
      </el-table-column>
    </el-table>
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="ID" prop="plId">
              <el-input-number clearable v-model.number="form.plId" controls-position="right" :placeholder="$t('btn.enter')+'ID'" :disabled="title==$t('btn.edit')"/>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="班组类别" prop="plLineType">
              <el-select v-model="form.plLineType" filterable clearable  :placeholder="$t('btn.select')+'班组类别'">
                <el-option v-for="item in  options.sys_line_type " :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="班组代码" prop="plLineCode">
              <el-input clearable v-model="form.plLineCode" :placeholder="$t('btn.enter')+'班组代码'" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="软件删除" prop="isDelete">
              <el-input clearable v-model="form.isDelete" :placeholder="$t('btn.enter')+'软件删除'"  :disabled="true"/>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="班组名称" prop="plLineName">
              <el-input clearable v-model="form.plLineName" :placeholder="$t('btn.enter')+'班组名称'" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
// 引入 pplines操作方法
import { listPpLines, addPpLines, delPpLines, updatePpLines, getPpLines, 
...
const columns = ref([
  ...
  { visible: false, prop: 'plLineName', label: '班组名称' },
])
  // 总条数
const total = ref(0)
  // 生产班组表格数据
