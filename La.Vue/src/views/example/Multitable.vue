...
<template>
  <div>
    ...
    <el-table :data="dataList" v-loading="loading" ref="table" border highlight-current-row @sort-change="sortChange"
      @selection-change="handleSelectionChange" height="302" style="width: 100%" @row-click="handleTableRow">
    ...
    <el-table :data="dataListSlv" v-loading="loading" ref="table" border highlight-current-row
      @sort-change="sortChangeSlv" @selection-change="handleSelectionChangeSlv" height="258" style="width: 100%">
      <el-table-column type="selection" width="50" align="center" />
      ...
      <el-table-column prop="esModel" label="机种" align="center" v-if="columns.showColumn('esModel')">
        <template #default="scope">
          <dict-tag :options=" options.sql_moc_item " :value="scope.row.esModel" />
        </template>
      </el-table-column>
      ...
      <el-table-column prop="esSubItem" label="子物料" align="center" v-if="columns.showColumn('esSubItem')">
        <template #default="scope">
          <dict-tag :options=" options.sql_moc_item " :value="scope.row.esSubItem" />
        </template>
      </el-table-column>
      ...
      <el-table-column prop="esNewItem" label="新物料" align="center" v-if="columns.showColumn('esNewItem')">
        <template #default="scope">
          <dict-tag :options=" options.sql_moc_item " :value="scope.row.esNewItem" />
        </template>
      </el-table-column>
      ...
      <el-table-column prop="esDeptEntryDate" label="部门录入日" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('esDeptEntryDate')" />
      <el-table-column prop="esDeptName" label="部门名称" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('esDeptName')" />
      ...
      <el-table-column prop="esPpLine" label="班组" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('esPpLine')" />
      ...
      <el-table-column prop="esPcbaLine" label="Pcba班组" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('esPcbaLine')" />
      ...
      <el-table-column prop="esDeptCreator" label="EsDeptCreator" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('esDeptCreator')" />
      <el-table-column prop="esDeptCreateTime" label="EsDeptCreateTime" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('esDeptCreateTime')" />
      <el-table-column prop="esDeptModifier" label="EsDeptModifier" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('esDeptModifier')" />
      <el-table-column prop="esDeptModifyTime" label="EsDeptModifyTime" align="center" :show-overflow-tooltip="true"
    ...
    <pagination class="mt10" background :total="totalSlv" v-model:page="queryParamsSlv.pageNum"
      v-model:limit="queryParamsSlv.pageSize" @pagination="getListSlv" />
    <!-- 添加或修改主设变对话框 -->
...
<script setup name="ppecmaster">
  ...
  // 总条数
  const totalSlv = ref(0)
  // 主设变表格数据
  const dataListSlv = ref([])
  // 查询参数
  const queryRef = ref()
  //定义起始时间
  ...
  function handleTableRow(evt, row) {

    //totalSlv.value = evt.emEcNo
    queryParamsSlv.esEcNo = evt.emEcNo
    getListSlv()
  }
  ...
  function getListSlv() {
    loading.value = true
    listPpEcSlave(queryParamsSlv).then(res => {
      const { code, data } = res
      if (code == 200) {
        dataListSlv.value = data.result
        totalSlv.value = data.totalNum
        loading.value = false
      }
    })
  }
  ...
  function handleQuery() {
    ...
    getList()
    queryParamsSlv.pageNum = 1
    queryParamsSlv.esEcNo = 'undefined'
    getListSlv()
  }
