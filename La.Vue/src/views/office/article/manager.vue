<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="plus" v-hasPermi="['system:article:add']"
    ...
    <el-table :data="dataList" ref="table" border>
      ...
      <el-table-column prop="coverUrl" label="文章封面" width="90" :show-overflow-tooltip="true">
        <template #default="{ row }">
          <el-image preview-teleported :src="row.coverUrl" :preview-src-list="[row.coverUrl]"
      ...
      <el-table-column sortable prop="status" align="center" label="状态" width="90">
        <template #default="scope">
          <el-tag :type="scope.row.status == '2' ? 'danger' : 'success'" disable-transitions>{{ scope.row.status == '2'
            ? '草稿' : '已发布' }}
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" width="230">
        <template #default="scope">
          ...
          <el-button text size="small" icon="edit" @click="handleUpdate(scope.row)"
            v-hasPermi="['system:article:update']">编辑</el-button>
