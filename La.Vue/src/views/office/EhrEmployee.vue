<!--
 * @Descripttion: (人事信息/ehr_employee)
 * @version: (1.0)
 * @Author: (Davis.Cheng)
 * @Date: (2023-05-04)
  ...
  <div>
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent>
      ...
      <el-form-item label="出生日期">
        <el-date-picker v-model="dateRangeEeBirthday" type="datetimerange" range-separator="-"
          :start-placeholder="$t('btn.dateStart')" :end-placeholder="$t('btn.dateEnd')"
          :placeholder="$t('btn.select')+'出生日期'" value-format="YYYY-MM-DD HH:mm:ss" :default-time="defaultTime"
      ...
      <el-form-item label="省区" prop="eeProvince">
        ...
        <el-select filterable clearable v-model="form.eeProvince" placeholder="省级" @change="changePr">
      ...
      <el-form-item label="入职日期">
        <el-date-picker v-model="dateRangeEeBeginDate" type="datetimerange" range-separator="-"
          :start-placeholder="$t('btn.dateStart')" :end-placeholder="$t('btn.dateEnd')"
          :placeholder="$t('btn.select')+'入职日期'" value-format="YYYY-MM-DD HH:mm:ss" :default-time="defaultTime"
          :shortcuts="dateOptions">
        </el-date-picker>
      </el-form-item>
      <el-form-item label="在职状态" prop="eeWorkState">
        <el-select filterable clearable v-model="queryParams.eeWorkState" :placeholder="$t('btn.select')+'在职状态'">
          <el-option v-for="item in  options.sys_workstate_list " :key="item.dictValue" :label="item.dictLabel"
            :value="item.dictValue">
            <span class="fl">{{ item.dictLabel }}</span>
            <span class="fr" style="color: var(--el-text-color-secondary);">{{ item.dictValue }}</span>
          </el-option>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" v-hasPermi="['ehr:employee:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" :disabled="single" v-hasPermi="['ehr:employee:edit']" plain icon="edit"
          @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" :disabled="multiple" v-hasPermi="['ehr:employee:delete']" plain icon="delete"
          @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button color="#DC143C" v-hasPermi="['ehr:employee:delete']" plain icon="delete" @click="handleClear">
          {{ $t('btn.clean') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button color="#00CED1" plain icon="Upload" @click="handleImport" v-hasPermi="['ehr:employee:import']">
          {{ $t('btn.import') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button color="#FF69B4" plain icon="download" @click="handleExport" v-hasPermi="['ehr:employee:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table :data="dataList" v-loading="loading" ref="table" border highlight-current-row @sort-change="sortChange"
      ...
      <el-table-column prop="eeId" label="ID" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('eeId')" />
      <el-table-column prop="eeName" label="姓名" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('eeName')" />
      <el-table-column prop="eeNameUsed" label="曾用名" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('eeNameUsed')" />
      <el-table-column prop="eeNickName" label="昵称" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('eeNickName')" />
      <el-table-column prop="eeEnglishName" label="英文名" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('eeEnglishName')" />
      ...
      <el-table-column prop="eeBirthday" label="出生日期" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('eeBirthday')" />
      <el-table-column prop="eeIdentityCard" label="身份证号" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('eeIdentityCard')" />
      ...
      <el-table-column prop="eeNativePlace" label="籍贯" align="center" v-if="columns.showColumn('eeNativePlace')">
        <template #default="scope">
          <dict-tag :options=" options.sys_country_list " :value="scope.row.eeNativePlace" />
        </template>
      </el-table-column>
      ...
      <el-table-column prop="eeEmail" label="邮件" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('eeEmail')" />
      <el-table-column prop="eePhone" label="电话" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('eePhone')" />
      ...
      <el-table-column prop="eeProvince" label="省区" align="center" v-if="columns.showColumn('eeProvince')">
        <template #default="scope">
          <dict-tag :options=" options.sys_country_list " :value="scope.row.eeProvince" />
        </template>
      </el-table-column>
      <el-table-column prop="eeCity" label="市区" align="center" v-if="columns.showColumn('eeCity')">
        <template #default="scope">
          <dict-tag :options=" options.sys_country_list " :value="scope.row.eeCity" />
        </template>
      </el-table-column>
      <el-table-column prop="eeCounty" label="县区" align="center" v-if="columns.showColumn('eeCounty')">
        <template #default="scope">
          <dict-tag :options=" options.sys_country_list " :value="scope.row.eeCounty" />
        </template>
      </el-table-column>
      <el-table-column prop="eeHomeAddress" label="家庭住址" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('eeHomeAddress')" />
      <el-table-column prop="eePostCode" label="邮编" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('eePostCode')" />
      ...
      <el-table-column prop="eeStayAddress" label="暂住地址" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('eeStayAddress')" />
      ...
      <el-table-column prop="eePostId" label="岗位" align="center" v-if="columns.showColumn('eePostId')">
        <template #default="scope">
          <dict-tag :options=" options.sql_dept_list " :value="scope.row.eePostId" />
        </template>
      </el-table-column>
      ...
      <el-table-column prop="eeEngageForm" label="聘用形式" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('eeEngageForm')" />
      ...
      <el-table-column prop="eeSchool" label="院校" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('eeSchool')" />
      <el-table-column prop="eeWorkID" label="工号" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('eeWorkID')" />
      <el-table-column prop="eeBeginDate" label="入职日期" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('eeBeginDate')" />
      ...
      <el-table-column prop="eeConversionTime" label="转正日期" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('eeConversionTime')" />
      <el-table-column prop="eeLeaveDate" label="离职日期" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('eeLeaveDate')" />
      <el-table-column prop="eeBeginContract" label="合同起始日" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('eeBeginContract')" />
      <el-table-column prop="eeBndContract" label="合同终止晶" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('eeBndContract')" />
      ...
      <el-table-column prop="eeAvatar" label="头像" align="center" v-if="columns.showColumn('eeAvatar')">
        <template #default="scope">
          <el-image preview-teleported :hide-on-click-modal="true" lazy class="table-td-thumb" fit="contain"
            :src="scope.row.eeAvatar" :preview-src-list="[scope.row.eeAvatar]">
            <div><el-icon :size="15">
                <document />
              </el-icon></div>
          </el-image>
      ...
      <el-table-column prop="eeQualificationAffix" label="学历附件" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('eeQualificationAffix')" />
      <el-table-column prop="eeTitleAffix" label="职称附件" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('eeTitleAffix')" />
      <el-table-column :label="$t('btn.operate')" align="center" width="160">
        <template #default="scope">
          <el-button type="primary" icon="view" @click="handlePreview(scope.row)"></el-button>
          <el-button v-hasPermi="['ehr:employee:edit']" type="success" icon="edit" :title="$t('btn.edit')"
            @click="handleUpdate(scope.row)"></el-button>
          <el-button v-hasPermi="['ehr:employee:delete']" type="danger" icon="delete" :title="$t('btn.delete')"
    ...
    <el-dialog :title="title" :lock-scroll="false" v-model="open">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        <el-tabs v-model="activeName" class="demo-tabs" @tab-click="handleClick">
          <el-tab-pane label="身份" name="first">
            ...
            <el-row :gutter="20">
              ...
              <el-col :lg="24">
                <el-form-item label="ID" prop="eeId">
                  <el-input clearable v-model="form.eeId" controls-position="right" :placeholder="$t('btn.enter')+'ID'"
                    disabled :disabled="title==$t('btn.edit')" />
                </el-form-item>
              </el-col>
              ...
              <el-col :lg="12">
                <el-form-item label="性别" prop="eeGender">
                  <el-select v-model="form.eeGender" filterable clearable :placeholder="$t('btn.select')+'性别'">
                    <el-option v-for="item in  options.sys_user_sex " :key="item.dictValue" :label="item.dictLabel"
              ...
              <el-col :lg="12">
                <el-form-item label="出生日期" prop="eeBirthday">
                  <el-date-picker clearable v-model="form.eeBirthday" type="datetime" :teleported="false"
              ...
              <el-col :lg="8">
                <el-form-item label="籍贯" prop="eeNativePlace">
                  ...
                  <el-select filterable clearable v-model="form.eeNativePlace" placeholder="省级" @change="changePr">
              ...
              <el-col :lg="12">
                <el-form-item label="电话" prop="eePhone">
                  <el-input clearable v-model="form.eePhone" :placeholder="$t('btn.enter')+'电话'" maxlength="11" />
                </el-form-item>
              </el-col>
              ...
              <el-col :lg="12">
                <el-form-item label="国家/地区" prop="eeCountry">
                  <el-select v-model="form.eeCountry" filterable clearable :placeholder="$t('btn.select')+'国家/地区'">
                    <el-option v-for="item in  options.sys_country_list " :key="item.dictValue" :label="item.dictLabel"
                      :value="item.dictValue"></el-option>
                  </el-select>
                </el-form-item>
              </el-col>
              ...
              <el-col :lg="8">
                <el-form-item label="市区" prop="eeCity">
                  <el-select filterable clearable v-model="form.eeCity" placeholder="市级" @change="changeCi">
              ...
              <el-col :lg="8">
                <el-form-item label="县区" prop="eeCounty">
                  <el-select filterable clearable v-model="form.eeCounty" placeholder="区级" @change="changeRe">
              ...
              <el-col :lg="12">
                <el-form-item label="户口性质" prop="eeHouseholdType">
                  <el-select v-model="form.eeHouseholdType" filterable clearable :placeholder="$t('btn.select')+'户口性质'">
                    <el-option v-for="item in  options.sys_household_type " :key="item.dictValue"
          ...
          <el-tab-pane label="入职" name="second">
            ...
            <el-row :gutter="20">
              ...
              <el-col :lg="8">
                <el-form-item label="招聘来源" prop="eeRecruited">
                  <el-select v-model="form.eeRecruited" filterable clearable :placeholder="$t('btn.select')+'招聘来源'">
                    <el-option v-for="item in  options.sys_recruited_list " :key="item.dictValue"
              ...
              <el-col :lg="8">
                <el-form-item label="入职日期" prop="eeBeginDate">
                  <el-date-picker clearable v-model="form.eeBeginDate" type="datetime" :teleported="false"
                    :placeholder="$t('btn.dateselect')"></el-date-picker>
                </el-form-item>
              </el-col>
              ...
              <el-col :lg="24">
                <el-form-item label="在职状态" prop="eeWorkState">
                  <el-radio-group v-model="form.eeWorkState">
                    <el-radio v-for="item in  options.sys_workstate_list " :key="item.dictValue"
          ...
          <el-tab-pane label="学历" name="third">
            <el-row :gutter="20">
              <el-col :lg="8">
                <el-form-item label="学历" prop="eeTiptopDegrEe">
              ...
              <el-col :lg="8">
                <el-form-item label="专业" prop="eeSpecialty">
                  <el-select v-model="form.eeSpecialty" filterable clearable :placeholder="$t('btn.select')+'专业'">
                    <el-option v-for="item in  options.sys_specialty_list " :key="item.dictValue"
                      :label="item.dictLabel" :value="parseInt(item.dictValue)"></el-option>
                  </el-select>
                </el-form-item>
              </el-col>
            ...
            <el-row :gutter="20">
              ...
              <el-col :lg="24">
                <el-form-item label="学历附件" prop="eeQualificationAffix">
                  <UploadFile clearable v-model="form.eeQualificationAffix" :fileType="['pdf']"
                    :data="{ uploadType: 1 }" />
                </el-form-item>
              </el-col>
              ...
              <el-col :lg="24">
                <el-form-item label="职称附件" prop="eeTitleAffix">
                  <UploadFile clearable v-model="form.eeTitleAffix" :fileType="['pdf']" :data="{ uploadType: 1 }" />
                </el-form-item>
              </el-col>
            </el-row>
            ...
            <el-row :gutter="20">
              <el-col :lg="12">
                <el-form-item label="试用期" prop="eeProbation">
                  <el-select v-model="form.eeProbation" filterable clearable :placeholder="$t('btn.select')+'试用期'">
                    <el-option v-for="item in  options.sys_trialterm_list " :key="item.dictValue"
                      :label="item.dictLabel" :value="item.dictValue"></el-option>
                  </el-select>
                </el-form-item>
              </el-col>
              ...
              <el-col :lg="12">
                <el-form-item label="转正日期" prop="eeConversionTime">
                  <el-date-picker clearable v-model="form.eeConversionTime" type="datetime" :teleported="false"
                    :placeholder="$t('btn.dateselect')"></el-date-picker>
                </el-form-item>
              </el-col>
              ...
              <el-col :lg="12">
                <el-form-item label="离职日期" prop="eeLeaveDate">
                  <el-date-picker clearable v-model="form.eeLeaveDate" type="datetime" :teleported="false"
                    :placeholder="$t('btn.dateselect')"></el-date-picker>
                </el-form-item>
              </el-col>
              ...
              <el-col :lg="12">
                <el-form-item label="合同起始日" prop="eeBeginContract">
                  <el-date-picker clearable v-model="form.eeBeginContract" type="datetime" :teleported="false"
                    :placeholder="$t('btn.dateselect')"></el-date-picker>
                </el-form-item>
              </el-col>
              ...
              <el-col :lg="12">
                <el-form-item label="合同终止日" prop="eeBndContract">
                  <el-date-picker clearable v-model="form.eeBndContract" type="datetime" :teleported="false"
                    :placeholder="$t('btn.dateselect')"></el-date-picker>
                </el-form-item>
              </el-col>
...
<script setup name="ehremployee">
  // 引入 ehremployee操作方法
  import {
    listEhrEmployee, addEhrEmployee, delEhrEmployee, updateEhrEmployee, getEhrEmployee,
    clearEhrEmployee,
  }
    from '@/api/office/ehremployee.js'
  ...
  function changePr() {
    for (var key of pacasv) {
      if (key.name == form.value.eeProvince) {
        cityList.value = key.children
        form.value.eeCity = key.children[0].name //默认选择第一个显示
  ...
  function changeCi() {
    cityList.value.forEach((item, index) => {
      if (item.name == form.value.eeCity) {
        regionList.value = item.children
        form.value.eeCounty = item.children[0].name //默认选择第一个显示
  ...
  function changeRe() {
    regionList.value.forEach((item, index) => {
      if (item.name == form.value.eeCounty) {
  ...
  const state = reactive({
    ...
    rules: {
      ...
      eeIdentityCard: [{
        ...
        message: "身份证号" + proxy.$t('btn.empty'),
        trigger: ['blur', 'change']
      }],
      ...
      eeCity: [{ required: true, message: "市区" + proxy.$t('btn.empty'), trigger: "change" }],
      eeCounty: [{ required: true, message: "县区" + proxy.$t('btn.empty'), trigger: "change" }],
  ...
  function handleAdd() {
    ...
    form.value.eeId = crypto.randomUUID()
    opertype.value = 1
  }
  ...
  function handleExport() {
    proxy
      ...
      .then(async () => {
        await proxy.downFile('/Office/EhrEmployee/export', { ...queryParams })
      })
  }
