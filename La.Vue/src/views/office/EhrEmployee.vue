...
<template>
  <div>
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button type="success" :disabled="single" v-hasPermi="['ehr:employee:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" :disabled="multiple" v-hasPermi="['ehr:employee:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button color="#666699" v-hasPermi="['ehr:employee:delete']" plain icon="BrushFilled" @click="handleClear">
         {{ $t('btn.clean') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
    ...
    <el-table :data="dataList" v-loading="loading" ref="table" border highlight-current-row @sort-change="sortChange" @selection-change="handleSelectionChange" height="602"
      ...
      <el-table-column prop="eeDutyName" label="职务" align="center" v-if="columns.showColumn('eeDutyName')">
        <template #default="scope">
          <dict-tag :options=" options.sql_posts_lit " :value="scope.row.eeDutyName" />
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('btn.operate')" align="center" width="160">
        <template #default="scope">
          ...
          <el-button v-hasPermi="['ehr:employee:edit']" type="success" icon="edit" :title="$t('btn.edit')" @click="handleUpdate(scope.row)"></el-button>
          <el-button v-hasPermi="['ehr:employee:delete']" type="danger" icon="delete" :title="$t('btn.delete')" @click="handleDelete(scope.row)"></el-button>
        </template>
      </el-table-column>
    </el-table>
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="市区" prop="eeCity">
              <el-select v-model="form.eeCity" filterable clearable  :placeholder="$t('btn.select')+'市区'">
                <el-option v-for="item in  options.sys_country_list " :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="县区" prop="eeCounty">
              <el-select v-model="form.eeCounty" filterable clearable  :placeholder="$t('btn.select')+'县区'">
                <el-option v-for="item in  options.sys_country_list " :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="职务" prop="eeDutyName">
              <el-select v-model="form.eeDutyName" filterable clearable  :placeholder="$t('btn.select')+'职务'">
                <el-option v-for="item in  options.sql_posts_lit " :key="item.dictValue" :label="item.dictLabel" :value="parseInt(item.dictValue)"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="聘用形式" prop="eeEngageForm">
              <el-input clearable v-model="form.eeEngageForm" :placeholder="$t('btn.enter')+'聘用形式'" />
            </el-form-item>
          </el-col>
...
// 引入 ehremployee操作方法
import { listEhrEmployee, addEhrEmployee, delEhrEmployee, updateEhrEmployee, getEhrEmployee, 
 clearEhrEmployee,  
 } 
