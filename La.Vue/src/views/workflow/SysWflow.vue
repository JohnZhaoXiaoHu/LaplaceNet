...
<template>
  <div>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" v-hasPermi="['sys:wflow:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" :disabled="single" v-hasPermi="['sys:wflow:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" :disabled="multiple" v-hasPermi="['sys:wflow:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
    ...
    <el-table :data="dataList" v-loading="loading" ref="table" border highlight-current-row @sort-change="sortChange" @selection-change="handleSelectionChange" height="602"
      ...
      <el-table-column prop="wflowId" label="Guid" align="center" v-if="columns.showColumn('wflowId')"/>
      ...
      <el-table-column :label="$t('btn.operate')" align="center" width="160">
        <template #default="scope">
          <el-button v-hasPermi="['sys:wflow:edit']" type="success" icon="edit" :title="$t('btn.edit')" @click="handleUpdate(scope.row)"></el-button>
          <el-button v-hasPermi="['sys:wflow:delete']" type="danger" icon="delete" :title="$t('btn.delete')" @click="handleDelete(scope.row)"></el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :lock-scroll="false" v-model="open" >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="Guid" prop="wflowId">
              <el-input-number clearable v-model.number="form.wflowId" controls-position="right" :placeholder="$t('btn.enter')+'Guid'" :disabled="title==$t('btn.edit')"/>
            </el-form-item>
          </el-col>
...
<script setup name="syswflow">
// 引入 syswflow操作方法
import { listSysWflow, addSysWflow, delSysWflow, updateSysWflow, getSysWflow, 
 
 } 
from '@/api/workflow/syswflow.js'
//获取当前组件实例
...
//字段显示控制
const columns = ref([
  { visible: true, prop: 'wflowId', label: 'Guid' },
...
function getList(){
  loading.value = true
  listSysWflow(queryParams).then(res => {
    const { code, data } = res
    if (code == 200) {
...
function handleSelectionChange(selection) {
  ids.value = selection.map((item) => item.wflowId);
...
function handleUpdate(row) {
  reset()
  const id = row.wflowId || ids.value
  getSysWflow(id).then((res) => {
    const { code, data } = res
    if (code == 200) {
      open.value = true
...
function submitForm() {
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      if (form.value.wflowId != undefined && opertype.value === 2) {
        updateSysWflow(form.value).then((res) => {
        ...
        addSysWflow(form.value).then((res) => {
