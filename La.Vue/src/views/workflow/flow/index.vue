...
<template>
  <div>
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button type="success" :disabled="single" v-hasPermi="['wf:flowscheme:edit']" plain icon="edit"
          @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" :disabled="multiple" v-hasPermi="['wf:flowscheme:delete']" plain icon="delete"
          @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
    ...
    <el-table :data="dataList" v-loading="loading" ref="table" border highlight-current-row @sort-change="sortChange"
      ...
      <el-table-column prop="schemeCode" label="流程编号" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('schemeCode')" />
      <el-table-column prop="schemeName" label="流程名称" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('schemeName')" />
      ...
      <el-table-column prop="schemeVersion" label="流程版本" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('schemeVersion')" />
      <el-table-column prop="schemeCanUser" label="流程使用人" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('schemeCanUser')" />
      <el-table-column prop="schemeContent" label="流程内容" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('schemeContent')" />
      ...
      <el-table-column prop="active" label="执行" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('active')" />
      ...
      <el-table-column :label="$t('btn.operate')" align="center" width="160">
        <template #default="scope">
          <el-button v-hasPermi="['wf:flowscheme:edit']" type="success" icon="edit" :title="$t('btn.edit')"
            @click="handleUpdate(scope.row)"></el-button>
          <el-button v-hasPermi="['wf:flowscheme:delete']" type="danger" icon="delete" :title="$t('btn.delete')"
          ...
          <el-col :lg="12">
            <el-form-item label="流程类型" prop="schemeType">
              <el-select v-model="form.schemeType" filterable clearable :placeholder="$t('btn.select')+'流程类型'">
                <el-option v-for="item in  options.schemeTypeOptions" :key="item.dictValue" :label="item.dictLabel"
          ...
          <el-col :lg="12">
            <el-form-item label="表单类型" prop="frmType">
              <el-select v-model="form.frmType" filterable clearable :placeholder="$t('btn.select')+'表单类型'">
                <el-option v-for="item in  options.frmTypeOptions" :key="item.dictValue" :label="item.dictLabel"
          ...
          <el-col :lg="12">
            <el-form-item label="权限类型" prop="authorizeType">
              <el-select v-model="form.authorizeType" filterable clearable :placeholder="$t('btn.select')+'权限类型'">
                <el-option v-for="item in  options.authorizeTypeOptions" :key="item.dictValue" :label="item.dictLabel"
                  :value="parseInt(item.dictValue)"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
...
<script setup name="wfflowscheme">
  // 引入 wfflowscheme操作方法
  import {
    listWfFlowscheme, addWfFlowscheme, delWfFlowscheme, updateWfFlowscheme, getWfFlowscheme,
