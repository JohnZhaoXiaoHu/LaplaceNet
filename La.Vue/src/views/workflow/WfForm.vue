...
<template>
  <div>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" v-hasPermi="['wf:form:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" :disabled="single" v-hasPermi="['wf:form:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" :disabled="multiple" v-hasPermi="['wf:form:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button color="#FF69B4" plain icon="download" @click="handleExport" v-hasPermi="['wf:form:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table :data="dataList" v-loading="loading" ref="table" border highlight-current-row @sort-change="sortChange" @selection-change="handleSelectionChange" height="602"
      ...
      <el-table-column prop="name" label="表单名称" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('name')"/>
      ...
      <el-table-column prop="webId" label="自定义表单" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('webId')"/>
      <el-table-column prop="fields" label="字段数" align="center" v-if="columns.showColumn('fields')"/>
      <el-table-column prop="contentData" label="字段" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('contentData')"/>
      <el-table-column prop="contentParse" label="字段格式化" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('contentParse')"/>
      <el-table-column prop="content" label="表单内容" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('content')"/>
      <el-table-column prop="dbName" label="数据库备用" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('dbName')"/>
      ...
      <el-table-column :label="$t('btn.operate')" align="center" width="160">
        <template #default="scope">
          <el-button v-hasPermi="['wf:form:edit']" type="success" icon="edit" :title="$t('btn.edit')" @click="handleUpdate(scope.row)"></el-button>
          <el-button v-hasPermi="['wf:form:delete']" type="danger" icon="delete" :title="$t('btn.delete')" @click="handleDelete(scope.row)"></el-button>
        </template>
      </el-table-column>
    </el-table>
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="表单名称" prop="name">
              <el-input clearable v-model="form.name" :placeholder="$t('btn.enter')+'表单名称'" />
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="自定义表单" prop="webId">
              <el-input clearable v-model="form.webId" :placeholder="$t('btn.enter')+'自定义表单'" />
            </el-form-item>
          </el-col>
    
          <el-col :lg="12">
            <el-form-item label="字段数" prop="fields">
              <el-input clearable v-model="form.fields" :placeholder="$t('btn.enter')+'字段数'" />
            </el-form-item>
          </el-col>

          <el-col :lg="24">
            <el-form-item label="字段" prop="contentData">
              <el-input clearable type="textarea" v-model="form.contentData" :placeholder="$t('btn.enter')+'字段'"/>
            </el-form-item>
          </el-col>

          <el-col :lg="24">
            <el-form-item label="字段格式化" prop="contentParse">
              <el-input clearable type="textarea" v-model="form.contentParse" :placeholder="$t('btn.enter')+'字段格式化'"/>
            </el-form-item>
          </el-col>

          <el-col :lg="24">
            <el-form-item label="表单内容" prop="content">
              <el-input clearable type="textarea" v-model="form.content" :placeholder="$t('btn.enter')+'表单内容'"/>
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="数据库备用" prop="dbName">
              <el-input clearable v-model="form.dbName" :placeholder="$t('btn.enter')+'数据库备用'" />
            </el-form-item>
          </el-col>
...
<script setup name="wfform">
// 引入 wfform操作方法
import { listWfForm, addWfForm, delWfForm, updateWfForm, getWfForm, 
 
 } 
from '@/api/workflow/wfform.js'
//获取当前组件实例
...
const columns = ref([
  ...
  { visible: true, prop: 'name', label: '表单名称' },
  { visible: true, prop: 'frmType', label: '表单类型' },
  { visible: true, prop: 'webId', label: '自定义表单' },
  { visible: true, prop: 'fields', label: '字段数' },
  { visible: true, prop: 'contentData', label: '字段' },
  { visible: true, prop: 'contentParse', label: '字段格式化' },
  { visible: true, prop: 'content', label: '表单内容' },
  { visible: false, prop: 'dbName', label: '数据库备用' },
...
function getList(){
  loading.value = true
  listWfForm(queryParams).then(res => {
    const { code, data } = res
    if (code == 200) {
  ...
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      if (form.value.id != undefined && opertype.value === 2) {
        updateWfForm(form.value).then((res) => {
...
function handleExport() {
  proxy
    .$confirm(proxy.$t('common.confirmExport')+"表单", proxy.$t('common.warningTips'), {
      confirmButtonText: proxy.$t('btn.submit'),
      cancelButtonText: proxy.$t('btn.cancel'),
      type: "warning",
    })
    .then(async () => {
      await proxy.downFile('/workflow/WfForm/export', { ...queryParams })
    })
}
