  ...
  <div>
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent>
      <el-form-item label="生效日期">
        <el-date-picker v-model="dateRangeErEffDate" type="datetimerange" range-separator="-"
          :start-placeholder="$t('btn.dateStart')" :end-placeholder="$t('btn.dateEnd')"
          :placeholder="$t('btn.select')+'生效日期'" value-format="YYYY-MM-DD HH:mm:ss" :default-time="defaultTime"
      ...
      <el-form-item label="To币别" prop="ertoCcy">
        <el-select filterable clearable v-model="queryParams.ertoCcy" :placeholder="$t('btn.select')+'To币别'">
          <el-option v-for="item in  options.sys_ccy_type " :key="item.dictValue" :label="item.dictLabel"
            :value="item.dictValue">
            <span class="fl">{{ item.dictLabel }}</span>
            <span class="fr" style="color: var(--el-text-color-secondary);">{{ item.dictValue }}</span>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" v-hasPermi="['la:ficoexchangerate:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" :disabled="single" v-hasPermi="['la:ficoexchangerate:edit']" plain icon="edit"
          @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" :disabled="multiple" v-hasPermi="['la:ficoexchangerate:delete']" plain icon="delete"
          @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button color="#00CED1" plain icon="Upload" @click="handleImport"
          v-hasPermi="['la:ficoexchangerate:import']">
          {{ $t('btn.import') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button color="#FF69B4" plain icon="download" @click="handleExport"
          v-hasPermi="['la:ficoexchangerate:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table :data="dataList" v-loading="loading" ref="table" border highlight-current-row @sort-change="sortChange"
      ...
      <el-table-column prop="erEffDate" label="生效日期" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('erEffDate')" />
      ...
      <el-table-column :label="$t('btn.operate')" align="center" width="160">
        <template #default="scope">
          <el-button v-hasPermi="['la:ficoexchangerate:edit']" type="success" icon="edit" :title="$t('btn.edit')"
            @click="handleUpdate(scope.row)"></el-button>
          <el-button v-hasPermi="['la:ficoexchangerate:delete']" type="danger" icon="delete" :title="$t('btn.delete')"
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="生效日期" prop="erEffDate">
              <el-date-picker clearable v-model="form.erEffDate" type="datetime" :teleported="false"
          ...
          <el-col :lg="24">
            <el-form-item label="基数" prop="erStd">
              <el-input-number clearable v-model.number="form.erStd" :controls="true" controls-position="right"
                :placeholder="$t('btn.enter')+'基数'" />
            </el-form-item>
          </el-col>
...
<script setup name="ficoexchangerate">
  ...
  import {
    listFicoExchangeRate, addFicoExchangeRate, delFicoExchangeRate, updateFicoExchangeRate, getFicoExchangeRate,
