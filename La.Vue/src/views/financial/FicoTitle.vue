  ...
  <div>
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent>
      ...
      <el-form-item label="科目名称" prop="ftTitleName">
        <el-input clearable v-model="queryParams.ftTitleName" :placeholder="$t('btn.enter')+'科目名称'" />
      </el-form-item>
      <el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" v-hasPermi="['la:ficotitle:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" :disabled="single" v-hasPermi="['la:ficotitle:edit']" plain icon="edit"
          @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" :disabled="multiple" v-hasPermi="['la:ficotitle:delete']" plain icon="delete"
          @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button color="#00CED1" plain icon="Upload" @click="handleImport" v-hasPermi="['la:ficotitle:import']">
          {{ $t('btn.import') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button color="#FF69B4" plain icon="download" @click="handleExport" v-hasPermi="['la:ficotitle:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table :data="dataList" v-loading="loading" ref="table" border highlight-current-row @sort-change="sortChange"
      ...
      <el-table-column prop="ftTitleCode" label="科目代码" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('ftTitleCode')" />
      <el-table-column prop="ftTitleName" label="科目名称" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('ftTitleName')" />
      <el-table-column prop="ftTitleLang" label="语言" align="center" v-if="columns.showColumn('ftTitleLang')">
        <template #default="scope">
          <dict-tag :options=" options.sys_lang_list " :value="scope.row.ftTitleLang" />
        </template>
      </el-table-column>
      <el-table-column :label="$t('btn.operate')" align="center" width="160">
        <template #default="scope">
          <el-button v-hasPermi="['la:ficotitle:edit']" type="success" icon="edit" :title="$t('btn.edit')"
            @click="handleUpdate(scope.row)"></el-button>
          <el-button v-hasPermi="['la:ficotitle:delete']" type="danger" icon="delete" :title="$t('btn.delete')"
    ...
    <el-dialog :title="title" :lock-scroll="false" v-model="open">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        <el-row :gutter="20">
          ...
          <el-col :lg="24">
            <el-form-item label="科目名称" prop="ftTitleName">
              <el-input clearable v-model="form.ftTitleName" :placeholder="$t('btn.enter')+'科目名称'" />
            </el-form-item>
          </el-col>

          <el-col :lg="24">
            <el-form-item label="语言" prop="ftTitleLang">
              <el-select v-model="form.ftTitleLang" filterable clearable :placeholder="$t('btn.select')+'语言'">
                <el-option v-for="item in  options.sys_lang_list " :key="item.dictValue" :label="item.dictLabel"
                  :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
...
<script setup name="ficotitle">
  // 引入 ficotitle操作方法
  import {
    listFicoTitle, addFicoTitle, delFicoTitle, updateFicoTitle, getFicoTitle,
  ...
  const state = reactive({
    ...
    rules: {
      ...
      ftTitleName: [{ required: true, message: "科目名称" + proxy.$t('btn.empty'), trigger: "blur" }],
      ftTitleLang: [{ required: true, message: "语言" + proxy.$t('btn.empty'), trigger: "change" }],
    },
    options: {
