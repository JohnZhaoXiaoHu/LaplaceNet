  ...
  <div>
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent>
      ...
      <el-form-item label="有效从">
        <el-date-picker v-model="dateRangeFpActDate" type="datetimerange" range-separator="-"
          :start-placeholder="$t('btn.dateStart')" :end-placeholder="$t('btn.dateEnd')"
          :placeholder="$t('btn.select')+'有效从'" value-format="YYYY-MM-DD HH:mm:ss" :default-time="defaultTime"
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" v-hasPermi="['la:ficoprctr:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" :disabled="single" v-hasPermi="['la:ficoprctr:edit']" plain icon="edit"
          @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" :disabled="multiple" v-hasPermi="['la:ficoprctr:delete']" plain icon="delete"
          @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      ...
      <el-col :span="1.5">
        <el-button color="#FF69B4" plain icon="download" @click="handleExport" v-hasPermi="['la:ficoprctr:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table :data="dataList" v-loading="loading" ref="table" border highlight-current-row @sort-change="sortChange"
      ...
      <el-table-column :label="$t('btn.operate')" align="center" width="160">
        <template #default="scope">
          <el-button v-hasPermi="['la:ficoprctr:edit']" type="success" icon="edit" :title="$t('btn.edit')"
            @click="handleUpdate(scope.row)"></el-button>
          <el-button v-hasPermi="['la:ficoprctr:delete']" type="danger" icon="delete" :title="$t('btn.delete')"
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="24">
            <el-form-item label="有效从" prop="fpActDate">
              <el-date-picker clearable v-model="form.fpActDate" type="datetime" :teleported="false"
          ...
          <el-col :lg="24">
            <el-form-item label="有效到" prop="fpExpDate">
              <el-date-picker clearable v-model="form.fpExpDate" type="datetime" :teleported="false"
                :placeholder="$t('btn.dateselect')"></el-date-picker>
            </el-form-item>
          </el-col>
