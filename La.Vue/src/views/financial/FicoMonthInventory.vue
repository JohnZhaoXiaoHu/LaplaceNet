<!--
 * @Descripttion: (月度存货/fico_month_inventory)
 * @version: (1.0)
 * @Author: (Davis.Cheng)
 * @Date: (2023-04-11)
  ...
  <div>
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent>
      <el-form-item label="工厂" prop="miPlant">
        <el-select filterable clearable v-model="queryParams.miPlant" :placeholder="$t('btn.select')+'工厂'">
      ...
      <el-form-item label="年月" prop="miYm">
        <el-select filterable clearable v-model="queryParams.miYm" :placeholder="$t('btn.select')+'年月'">
      ...
      <el-form-item label="物料" prop="miItem">
        <el-select filterable clearable v-model="queryParams.miItem" :placeholder="$t('btn.select')+'物料'">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" v-hasPermi="['la:ficomonthinventory:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" :disabled="single" v-hasPermi="['la:ficomonthinventory:edit']" plain icon="edit"
          @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" :disabled="multiple" v-hasPermi="['la:ficomonthinventory:delete']" plain icon="delete"
          @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button color="#00CED1" plain icon="Upload" @click="handleImport"
          v-hasPermi="['la:ficomonthinventory:import']">
          {{ $t('btn.import') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button color="#FF69B4" plain icon="download" @click="handleExport"
          v-hasPermi="['la:ficomonthinventory:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table :data="dataList" v-loading="loading" ref="table" border highlight-current-row @sort-change="sortChange"
      ...
      <el-table-column prop="miId" label="ID" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('miId')" />
      ...
      <el-table-column prop="miItem" label="物料" align="center" v-if="columns.showColumn('miItem')">
        <template #default="scope">
          <dict-tag :options=" options.sql_moc_item " :value="scope.row.miItem" />
        </template>
      </el-table-column>
      ...
      <el-table-column prop="miLocalCcy" label="币种" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('miLocalCcy')" />
      ...
      <el-table-column prop="miBalancedate" label="登录日期" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('miBalancedate')" />
      <el-table-column :label="$t('btn.operate')" align="center" width="160">
        <template #default="scope">
          <el-button v-hasPermi="['la:ficomonthinventory:edit']" type="success" icon="edit" :title="$t('btn.edit')"
            @click="handleUpdate(scope.row)"></el-button>
          <el-button v-hasPermi="['la:ficomonthinventory:delete']" type="danger" icon="delete" :title="$t('btn.delete')"
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="ID" prop="miId">
              <el-input clearable v-model="form.miId" controls-position="right" :placeholder="$t('btn.enter')+'ID'"
          ...
          <el-col :lg="12">
            <el-form-item label="工厂" prop="miPlant">
              <el-select v-model="form.miPlant" filterable clearable :placeholder="$t('btn.select')+'工厂'">
                <el-option v-for="item in  options.sys_plant_list " :key="item.dictValue" :label="item.dictLabel"
          ...
          <el-col :lg="12">
            <el-form-item label="期间" prop="miFy">
              <el-select v-model="form.miFy" filterable clearable :placeholder="$t('btn.select')+'期间'">
          ...
          <el-col :lg="12">
            <el-form-item label="年月" prop="miYm">
              <el-select v-model="form.miYm" filterable clearable :placeholder="$t('btn.select')+'年月'">
                <el-option v-for="item in  options.sql_ym_list " :key="item.dictValue" :label="item.dictLabel"
                  :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          ...
          <el-col :lg="12">
            <el-form-item label="物料" prop="miItem">
              <el-select v-model="form.miItem" filterable clearable :placeholder="$t('btn.select')+'物料'">
                <el-option v-for="item in  options.sql_moc_item " :key="item.dictValue" :label="item.dictLabel"
                  :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          ...
          <el-col :lg="12">
            <el-form-item label="评估类" prop="miValType">
              <el-select v-model="form.miValType" filterable clearable :placeholder="$t('btn.select')+'评估类'">
                <el-option v-for="item in  options.sys_fin_val " :key="item.dictValue" :label="item.dictLabel"
                  :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="价格控制" prop="miPriceControl">
              <el-input clearable v-model="form.miPriceControl" :placeholder="$t('btn.enter')+'价格控制'" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="移动平均价" prop="miMovingAverage">
              <el-input clearable v-model="form.miMovingAverage" :placeholder="$t('btn.enter')+'移动平均价'" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="Per单位" prop="miPerUnit">
              <el-input clearable v-model="form.miPerUnit" :placeholder="$t('btn.enter')+'Per单位'" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="币种" prop="miLocalCcy">
              <el-input clearable v-model="form.miLocalCcy" :placeholder="$t('btn.enter')+'币种'" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="库存" prop="miInventoryQty">
              <el-input clearable v-model="form.miInventoryQty" :placeholder="$t('btn.enter')+'库存'" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="金额" prop="miInventoryAmount">
              <el-input clearable v-model="form.miInventoryAmount" :placeholder="$t('btn.enter')+'金额'" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="登录日期" prop="miBalancedate">
              <el-date-picker clearable v-model="form.miBalancedate" type="datetime" :teleported="false"
...
<script setup name="ficomonthinventory">
  ...
  import {
    listFicoMonthInventory, addFicoMonthInventory, delFicoMonthInventory, updateFicoMonthInventory, getFicoMonthInventory,
  ...
  const state = reactive({
    ...
    rules: {
      ...
      miLocalCcy: [{ required: true, message: "币种" + proxy.$t('btn.empty'), trigger: "blur" }],
