...
<template>
  <div>
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent>
      ...
      <el-form-item label="物料" prop="miItem">
        <el-select clearable v-model="queryParams.miItem" placeholder="请选择物料">
          <el-option v-for="item in  options.sql_mats_list " :key="item.dictValue" :label="item.dictLabel"
            :value="item.dictValue">
            <span class="fl">{{ item.dictLabel }}</span>
            <span class="fr" style="color: var(--el-text-color-secondary);">{{ item.dictValue }}</span>
          </el-option>
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button type="success" :disabled="single" v-hasPermi="['fico:monthinventory:edit']" plain icon="edit"
          @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" :disabled="multiple" v-hasPermi="['fico:monthinventory:delete']" plain icon="delete"
          @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button color="#FF69B4" plain icon="download" @click="handleExport"
          v-hasPermi="['fico:monthinventory:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table :data="dataList" v-loading="loading" ref="table" border highlight-current-row @sort-change="sortChange"
      ...
      <el-table-column :label="$t('btn.operate')" align="center" width="160">
        <template #default="scope">
          <el-button v-hasPermi="['fico:monthinventory:edit']" type="success" icon="edit" :title="$t('btn.edit')"
            @click="handleUpdate(scope.row)"></el-button>
          <el-button v-hasPermi="['fico:monthinventory:delete']" type="danger" icon="delete" :title="$t('btn.delete')"
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="24">
            <el-form-item label="ID" prop="miId">
              <el-input v-model="form.miId" controls-position="right" :placeholder="$t('btn.enter')+'ID'" disabled
          ...
          <el-col :lg="12">
            <el-form-item label="工厂" prop="miPlant">
              <el-select v-model="form.miPlant" :placeholder="$t('btn.select')+'工厂'" disabled>
                <el-option v-for="item in  options.sys_plant_list " :key="item.dictValue" :label="item.dictLabel"
          ...
          <el-col :lg="12">
            <el-form-item label="期间" prop="miFy">
              <el-select v-model="form.miFy" :placeholder="$t('btn.select')+'期间'" disabled>
                <el-option v-for="item in  options.sql_period_list " :key="item.dictValue" :label="item.dictLabel"
                  :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          ...
          <el-col :lg="12">
            <el-form-item label="年月" prop="miYm">
              <el-select v-model="form.miYm" :placeholder="$t('btn.select')+'年月'" disabled>
                <el-option v-for="item in  options.sql_ym_list " :key="item.dictValue" :label="item.dictLabel"
                  :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          ...
          <el-col :lg="12">
            <el-form-item label="物料" prop="miItem">
              <el-select v-model="form.miItem" :placeholder="$t('btn.select')+'物料'" disabled>
                <el-option v-for="item in  options.sql_mats_list " :key="item.dictValue" :label="item.dictLabel"
                  :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          ...
          <el-col :lg="12">
            <el-form-item label="评估类" prop="miValType">
              <el-select v-model="form.miValType" :placeholder="$t('btn.select')+'评估类'" disabled>
                <el-option v-for="item in  options.sys_fin_val " :key="item.dictValue" :label="item.dictLabel"
                  :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          ...
          <el-col :lg="12">
            <el-form-item label="价格控制" prop="miPriceControl">
              <el-input v-model="form.miPriceControl" :placeholder="$t('btn.enter')+'价格控制'" disabled />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="移动平均价" prop="miMovingAverage">
              <el-input v-model="form.miMovingAverage" :placeholder="$t('btn.enter')+'移动平均价'" disabled />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="Per单位" prop="miPerUnit">
              <el-input v-model="form.miPerUnit" :placeholder="$t('btn.enter')+'Per单位'" disabled />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="币种" prop="miLocalCcy">
              <el-select v-model="form.miLocalCcy" :placeholder="$t('btn.select')+'币种'" disabled>
                <el-option v-for="item in  options.sys_ccy_type " :key="item.dictValue" :label="item.dictLabel"
                  :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          ...
          <el-col :lg="12">
            <el-form-item label="库存" prop="miInventoryQty">
              <el-input v-model="form.miInventoryQty" :placeholder="$t('btn.enter')+'库存'" disabled />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="金额" prop="miInventoryAmount">
              <el-input v-model="form.miInventoryAmount" :placeholder="$t('btn.enter')+'金额'" disabled />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="登录日期" prop="miBalancedate">
              <el-date-picker v-model="form.miBalancedate" type="datetime" :teleported="false" disabled
