  ...
  <div>
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent>
      <el-form-item label="财年" prop="fpFy">
        <el-input clearable v-model="queryParams.fpFy" :placeholder="$t('btn.enter')+'财年'" />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" v-hasPermi="['la:ficoperiod:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" :disabled="single" v-hasPermi="['la:ficoperiod:edit']" plain icon="edit"
          @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" :disabled="multiple" v-hasPermi="['la:ficoperiod:delete']" plain icon="delete"
          @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button color="#00CED1" plain icon="Upload" @click="handleImport" v-hasPermi="['la:ficoperiod:import']">
          {{ $t('btn.import') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button color="#FF69B4" plain icon="download" @click="handleExport" v-hasPermi="['la:ficoperiod:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table :data="dataList" v-loading="loading" ref="table" border highlight-current-row @sort-change="sortChange"
      ...
      <el-table-column prop="fpFy" label="财年" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('fpFy')" />
      <el-table-column prop="fpYm" label="年月" align="center" :show-overflow-tooltip="true"
        v-if="columns.showColumn('fpYm')" />
      ...
      <el-table-column :label="$t('btn.operate')" align="center" width="160">
        <template #default="scope">
          <el-button v-hasPermi="['la:ficoperiod:edit']" type="success" icon="edit" :title="$t('btn.edit')"
            @click="handleUpdate(scope.row)"></el-button>
          <el-button v-hasPermi="['la:ficoperiod:delete']" type="danger" icon="delete" :title="$t('btn.delete')"
...
<script setup name="ficoperiod">
  // 引入 ficoperiod操作方法
  import {
    listFicoPeriod, addFicoPeriod, delFicoPeriod, updateFicoPeriod, getFicoPeriod,
