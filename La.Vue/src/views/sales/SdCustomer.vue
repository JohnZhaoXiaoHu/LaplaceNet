...
<template>
  <div>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" v-hasPermi="['sd:customer:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" :disabled="single" v-hasPermi="['sd:customer:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" :disabled="multiple" v-hasPermi="['sd:customer:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button color="#FF69B4" plain icon="download" @click="handleExport" v-hasPermi="['sd:customer:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table :data="dataList" v-loading="loading" ref="table" border highlight-current-row @sort-change="sortChange" @selection-change="handleSelectionChange" height="602"
      ...
      <el-table-column prop="scPlant" label="销售工厂" align="center" v-if="columns.showColumn('scPlant')">
        <template #default="scope">
          <dict-tag :options=" options.sys_plant_list " :value="scope.row.scPlant" />
        </template>
      </el-table-column>
      <el-table-column prop="scIndustryType" label="行业类别" align="center" v-if="columns.showColumn('scIndustryType')">
        <template #default="scope">
          <dict-tag :options=" options.sys_nature_list " :value="scope.row.scIndustryType" />
        </template>
      </el-table-column>
      <el-table-column prop="scEnterpriseNature" label="企业性质" align="center" v-if="columns.showColumn('scEnterpriseNature')">
        <template #default="scope">
          <dict-tag :options=" options.sys_nature_list " :value="scope.row.scEnterpriseNature" />
        </template>
      </el-table-column>
      ...
      <el-table-column prop="scTaxType" label="税别" align="center" v-if="columns.showColumn('scTaxType')">
        <template #default="scope">
          <dict-tag :options=" options.sys_tax_list " :value="scope.row.scTaxType" />
        </template>
      </el-table-column>
      ...
      <el-table-column prop="scCcy" label="交易币种" align="center" v-if="columns.showColumn('scCcy')">
        <template #default="scope">
          <dict-tag :options=" options.sys_ccy_type " :value="scope.row.scCcy" />
        </template>
      </el-table-column>
      <el-table-column prop="scPayTerms" label="付款条件" align="center" v-if="columns.showColumn('scPayTerms')">
        <template #default="scope">
          <dict-tag :options=" options.sys_payment_terms " :value="scope.row.scPayTerms" />
        </template>
      </el-table-column>
      <el-table-column prop="scPayMethod" label="付款方式" align="center" v-if="columns.showColumn('scPayMethod')">
        <template #default="scope">
          <dict-tag :options=" options.sys_payment_method " :value="scope.row.scPayMethod" />
        </template>
      </el-table-column>
      ...
      <el-table-column prop="scCustomerGrade" label="客户等级" align="center" v-if="columns.showColumn('scCustomerGrade')">
        <template #default="scope">
          <dict-tag :options=" options.sys_grade_list " :value="scope.row.scCustomerGrade" />
        </template>
      </el-table-column>
      <el-table-column prop="scCustomerCredit" label="客户信用" align="center" v-if="columns.showColumn('scCustomerCredit')">
        <template #default="scope">
          <dict-tag :options=" options.sys_credit_list " :value="scope.row.scCustomerCredit" />
        </template>
      </el-table-column>
      ...
      <el-table-column prop="scBankState" label="银行所在州省" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('scBankState')"/>
      ...
      <el-table-column prop="isFroze" label="冻结标记" align="center" v-if="columns.showColumn('isFroze')">
        <template #default="scope">
          <dict-tag :options=" options.sys_flag_list " :value="scope.row.isFroze" />
        </template>
      </el-table-column>
      <el-table-column :label="$t('btn.operate')" align="center" width="160">
        <template #default="scope">
          <el-button v-hasPermi="['sd:customer:edit']" type="success" icon="edit" :title="$t('btn.edit')" @click="handleUpdate(scope.row)"></el-button>
          <el-button v-hasPermi="['sd:customer:delete']" type="danger" icon="delete" :title="$t('btn.delete')" @click="handleDelete(scope.row)"></el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :lock-scroll="false" v-model="open" >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="销售组织" prop="scOrg">
              <el-select v-model="form.scOrg" filterable clearable  :placeholder="$t('btn.select')+'销售组织'">
          ...
          <el-col :lg="12">
            <el-form-item label="销售工厂" prop="scPlant">
              <el-select v-model="form.scPlant" filterable clearable  :placeholder="$t('btn.select')+'销售工厂'">
          ...
          <el-col :lg="12">
            <el-form-item label="行业类别" prop="scIndustryType">
              <el-select v-model="form.scIndustryType" filterable clearable  :placeholder="$t('btn.select')+'行业类别'">
                <el-option v-for="item in  options.sys_nature_list " :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="企业性质" prop="scEnterpriseNature">
              <el-select v-model="form.scEnterpriseNature" filterable clearable  :placeholder="$t('btn.select')+'企业性质'">
                <el-option v-for="item in  options.sys_nature_list " :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="税别" prop="scTaxType">
              <el-select v-model="form.scTaxType" filterable clearable  :placeholder="$t('btn.select')+'税别'">
                <el-option v-for="item in  options.sys_tax_list " :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="交易币种" prop="scCcy">
              <el-select v-model="form.scCcy" filterable clearable  :placeholder="$t('btn.select')+'交易币种'">
          ...
          <el-col :lg="12">
            <el-form-item label="付款条件" prop="scPayTerms">
              <el-select v-model="form.scPayTerms" filterable clearable  :placeholder="$t('btn.select')+'付款条件'">
                <el-option v-for="item in  options.sys_payment_terms " :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="付款方式" prop="scPayMethod">
              <el-select v-model="form.scPayMethod" filterable clearable  :placeholder="$t('btn.select')+'付款方式'">
                <el-option v-for="item in  options.sys_payment_method " :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="客户等级" prop="scCustomerGrade">
              <el-select v-model="form.scCustomerGrade" filterable clearable  :placeholder="$t('btn.select')+'客户等级'">
                <el-option v-for="item in  options.sys_grade_list " :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="客户信用" prop="scCustomerCredit">
              <el-select v-model="form.scCustomerCredit" filterable clearable  :placeholder="$t('btn.select')+'客户信用'">
                <el-option v-for="item in  options.sys_credit_list " :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
...
// 引入 sdcustomer操作方法
import { listSdCustomer, addSdCustomer, delSdCustomer, updateSdCustomer, getSdCustomer, 
 
 } 
from '@/api/sales/sdcustomer.js'
//获取当前组件实例
...
const state = reactive({
  form: {},
  rules: {
    scOrg: [{ required: true, message: "销售组织"+proxy.$t('btn.empty'), trigger: "change" }],
    scPlant: [{ required: true, message: "销售工厂"+proxy.$t('btn.empty'), trigger: "change" }],
    scIndustryType: [{ required: true, message: "行业类别"+proxy.$t('btn.empty'), trigger: "change" }],
    scEnterpriseNature: [{ required: true, message: "企业性质"+proxy.$t('btn.empty'), trigger: "change" }],
    ...
    scCcy: [{ required: true, message: "交易币种"+proxy.$t('btn.empty'), trigger: "change" }],
    scPayTerms: [{ required: true, message: "付款条件"+proxy.$t('btn.empty'), trigger: "change" }],
    scPayMethod: [{ required: true, message: "付款方式"+proxy.$t('btn.empty'), trigger: "change" }],
  ...
  options: {
    ...
    sys_crop_list: [],
    // 销售工厂 选项列表 格式 eg:{ dictLabel: '标签', dictValue: '0'}
    sys_plant_list: [],
...
function handleExport() {
  proxy
    .$confirm(proxy.$t('common.confirmExport')+"顾客信息", proxy.$t('common.warningTips'), {
      confirmButtonText: proxy.$t('btn.submit'),
      cancelButtonText: proxy.$t('btn.cancel'),
      type: "warning",
    })
    .then(async () => {
      await proxy.downFile('/sales/SdCustomer/export', { ...queryParams })
    })
}
