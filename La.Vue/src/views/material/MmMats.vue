  ...
  <div>
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent>
      <el-form-item label="工厂" prop="mmPlnt">
        <el-select filterable clearable  v-model="queryParams.mmPlnt" :placeholder="$t('btn.select')+'工厂'">
      ...
      <el-form-item label="物料" prop="mmMatItem">
        <el-input clearable v-model="queryParams.mmMatItem" :placeholder="$t('btn.enter')+'物料'" />
      </el-form-item>
      <el-form-item label="行业类别" prop="mmIndustry">
        <el-select filterable clearable  v-model="queryParams.mmIndustry" :placeholder="$t('btn.select')+'行业类别'">
      ...
      <el-form-item label="物料类型" prop="mmMatType">
        <el-select filterable clearable  v-model="queryParams.mmMatType" :placeholder="$t('btn.select')+'物料类型'">
          <el-option v-for="item in  options.sys_mats_type " :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue">
            <span class="fl">{{ item.dictLabel }}</span>
            <span class="fr" style="color: var(--el-text-color-secondary);">{{ item.dictValue }}</span>          
          </el-option>
        </el-select>
      ...
      <el-form-item label="标准单位" prop="mmBaseUnit">
        <el-select filterable clearable  v-model="queryParams.mmBaseUnit" :placeholder="$t('btn.select')+'标准单位'">
      ...
      <el-form-item label="物料组" prop="mmMatGroup">
        <el-select filterable clearable  v-model="queryParams.mmMatGroup" :placeholder="$t('btn.select')+'物料组'">
          <el-option v-for="item in  options.sys_mats_group " :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue">
            <span class="fl">{{ item.dictLabel }}</span>
            <span class="fr" style="color: var(--el-text-color-secondary);">{{ item.dictValue }}</span>          
          </el-option>
        </el-select>
      ...
      <el-form-item label="采购组" prop="mmPurGroup">
        <el-select filterable clearable  v-model="queryParams.mmPurGroup" :placeholder="$t('btn.select')+'采购组'">
      ...
      <el-form-item label="采购类型" prop="mmPurType">
        <el-select filterable clearable  v-model="queryParams.mmPurType" :placeholder="$t('btn.select')+'采购类型'">
      ...
      <el-form-item label="利润中心" prop="mmProfitCenter">
        <el-select filterable clearable  v-model="queryParams.mmProfitCenter" :placeholder="$t('btn.select')+'利润中心'">
      ...
      <el-form-item label="评估类" prop="mmValType">
        <el-select filterable clearable  v-model="queryParams.mmValType" :placeholder="$t('btn.select')+'评估类'">
      ...
      <el-form-item label="币种" prop="mmCcy">
        <el-select filterable clearable  v-model="queryParams.mmCcy" :placeholder="$t('btn.select')+'币种'">
      ...
      <el-form-item label="采购仓库" prop="mmSLoc">
        <el-select filterable clearable  v-model="queryParams.mmSLoc" :placeholder="$t('btn.select')+'采购仓库'">
      ...
      <el-form-item label="仓位" prop="mmLocPosn">
        <el-input clearable v-model="queryParams.mmLocPosn" :placeholder="$t('btn.enter')+'仓位'" />
      </el-form-item>
      <el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" v-hasPermi="['mm:mats:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" :disabled="single" v-hasPermi="['mm:mats:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" :disabled="multiple" v-hasPermi="['mm:mats:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button color="#00CED1" plain icon="Upload" @click="handleImport" v-hasPermi="['mm:mats:import']">
          {{ $t('btn.import') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button color="#FF69B4" plain icon="download" @click="handleExport" v-hasPermi="['mm:mats:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
    ...
    <el-table :data="dataList" v-loading="loading" ref="table" border highlight-current-row @sort-change="sortChange" @selection-change="handleSelectionChange" height="602"
      ...
      <el-table-column prop="mmId" label="ID" align="center" v-if="columns.showColumn('mmId')"/>
      <el-table-column prop="mmLastDate" label="更新日期" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('mmLastDate')"/>
      <el-table-column prop="mmPlnt" label="工厂" align="center" v-if="columns.showColumn('mmPlnt')">
        <template #default="scope">
          <dict-tag :options=" options.sys_plant_list " :value="scope.row.mmPlnt" />
        </template>
      </el-table-column>
      <el-table-column prop="mmMatItem" label="物料" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('mmMatItem')"/>
      ...
      <el-table-column prop="mmMatType" label="物料类型" align="center" v-if="columns.showColumn('mmMatType')">
        <template #default="scope">
          <dict-tag :options=" options.sys_mats_type " :value="scope.row.mmMatType" />
        </template>
      </el-table-column>
      <el-table-column prop="mmMatDes" label="物料文本" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('mmMatDes')"/>
      ...
      <el-table-column prop="mmProLevel" label="层次" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('mmProLevel')"/>
      <el-table-column prop="mmMatGroup" label="物料组" align="center" v-if="columns.showColumn('mmMatGroup')">
        <template #default="scope">
          <dict-tag :options=" options.sys_mats_group " :value="scope.row.mmMatGroup" />
        </template>
      </el-table-column>
      <el-table-column prop="mmPurGroup" label="采购组" align="center" v-if="columns.showColumn('mmPurGroup')">
        <template #default="scope">
          <dict-tag :options=" options.sys_pur_group " :value="scope.row.mmPurGroup" />
        </template>
      </el-table-column>
      <el-table-column prop="mmPurType" label="采购类型" align="center" v-if="columns.showColumn('mmPurType')">
        <template #default="scope">
          <dict-tag :options=" options.sys_pur_type " :value="scope.row.mmPurType" />
        </template>
      </el-table-column>
      <el-table-column prop="mmSpecType" label="特殊采购类" align="center" v-if="columns.showColumn('mmSpecType')">
        <template #default="scope">
          <dict-tag :options=" options.sys_pur_spec " :value="scope.row.mmSpecType" />
        </template>
      </el-table-column>
      <el-table-column prop="mmBulkMat" label="散装物料" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('mmBulkMat')"/>
      ...
      <el-table-column prop="mmisChecked" label="检验否" align="center" v-if="columns.showColumn('mmisChecked')">
        <template #default="scope">
          <dict-tag :options=" options.sys_flag_list " :value="scope.row.mmisChecked" />
        </template>
      </el-table-column>
      <el-table-column prop="mmProfitCenter" label="利润中心" align="center" v-if="columns.showColumn('mmProfitCenter')">
        <template #default="scope">
          <dict-tag :options=" options.sql_prctr_list " :value="scope.row.mmProfitCenter" />
        </template>
      </el-table-column>
      <el-table-column prop="mmDiffCode" label="差异码" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('mmDiffCode')"/>
      <el-table-column prop="mmisLot" label="批次管理" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('mmisLot')"/>
      <el-table-column prop="mmMpn" label="制造商物料" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('mmMpn')"/>
      <el-table-column prop="mmMfrs" label="制造商" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('mmMfrs')"/>
      <el-table-column prop="mmValType" label="评估类" align="center" v-if="columns.showColumn('mmValType')">
        <template #default="scope">
          <dict-tag :options=" options.sys_fin_val " :value="scope.row.mmValType" />
        </template>
      </el-table-column>
      ...
      <el-table-column prop="mmCcy" label="币种" align="center" v-if="columns.showColumn('mmCcy')">
        <template #default="scope">
          <dict-tag :options=" options.sys_ccy_type " :value="scope.row.mmCcy" />
        </template>
      </el-table-column>
      <el-table-column prop="mmPriceControl" label="价格控制" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('mmPriceControl')"/>
      ...
      <el-table-column prop="mmSLoc" label="采购仓库" align="center" v-if="columns.showColumn('mmSLoc')">
        <template #default="scope">
          <dict-tag :options=" options.sys_sloc_list " :value="scope.row.mmSLoc" />
        </template>
      </el-table-column>
      <el-table-column prop="mmESLoc" label="生产仓库" align="center" v-if="columns.showColumn('mmESLoc')">
        <template #default="scope">
          <dict-tag :options=" options.sys_sloc_list " :value="scope.row.mmESLoc" />
        </template>
      </el-table-column>
      <el-table-column prop="mmLocPosn" label="仓位" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('mmLocPosn')"/>
      <el-table-column prop="mmInventory" label="库存" align="center" v-if="columns.showColumn('mmInventory')"/>
      <el-table-column prop="mmLocEol" label="EOL" align="center" :show-overflow-tooltip="true" v-if="columns.showColumn('mmLocEol')"/>
      ...
      <el-table-column :label="$t('btn.operate')" align="center" width="160">
        <template #default="scope">
          <el-button v-hasPermi="['mm:mats:edit']" type="success" icon="edit" :title="$t('btn.edit')" @click="handleUpdate(scope.row)"></el-button>
          <el-button v-hasPermi="['mm:mats:delete']" type="danger" icon="delete" :title="$t('btn.delete')" @click="handleDelete(scope.row)"></el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :lock-scroll="false" v-model="open" >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        <el-row :gutter="20">
          ...
          <el-col :lg="12">
            <el-form-item label="ID" prop="mmId">
              <el-input-number clearable v-model.number="form.mmId" controls-position="right" :placeholder="$t('btn.enter')+'ID'" :disabled="title==$t('btn.edit')"/>
            </el-form-item>
          </el-col>


          <el-col :lg="12">
            <el-form-item label="更新日期" prop="mmLastDate">
              <el-date-picker clearable v-model="form.mmLastDate" type="datetime" :teleported="false" :placeholder="$t('btn.dateselect')"></el-date-picker>
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="工厂" prop="mmPlnt">
              <el-select v-model="form.mmPlnt" filterable clearable  :placeholder="$t('btn.select')+'工厂'">
          ...
          <el-col :lg="12">
            <el-form-item label="物料" prop="mmMatItem">
              <el-input clearable v-model="form.mmMatItem" :placeholder="$t('btn.enter')+'物料'" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="行业类别" prop="mmIndustry">
              <el-select v-model="form.mmIndustry" filterable clearable  :placeholder="$t('btn.select')+'行业类别'">
          ...
          <el-col :lg="12">
            <el-form-item label="物料类型" prop="mmMatType">
              <el-select v-model="form.mmMatType" filterable clearable  :placeholder="$t('btn.select')+'物料类型'">
                <el-option v-for="item in  options.sys_mats_type " :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="物料文本" prop="mmMatDes">
              <el-input clearable v-model="form.mmMatDes" :placeholder="$t('btn.enter')+'物料文本'" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="标准单位" prop="mmBaseUnit">
              <el-select v-model="form.mmBaseUnit" filterable clearable  :placeholder="$t('btn.select')+'标准单位'">
          ...
          <el-col :lg="12">
            <el-form-item label="层次" prop="mmProLevel">
              <el-input clearable v-model="form.mmProLevel" :placeholder="$t('btn.enter')+'层次'" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="物料组" prop="mmMatGroup">
              <el-select v-model="form.mmMatGroup" filterable clearable  :placeholder="$t('btn.select')+'物料组'">
                <el-option v-for="item in  options.sys_mats_group " :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="采购组" prop="mmPurGroup">
              <el-select v-model="form.mmPurGroup" filterable clearable  :placeholder="$t('btn.select')+'采购组'">
                <el-option v-for="item in  options.sys_pur_group " :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="采购类型" prop="mmPurType">
              <el-select v-model="form.mmPurType" filterable clearable  :placeholder="$t('btn.select')+'采购类型'">
          ...
          <el-col :lg="12">
            <el-form-item label="特殊采购类" prop="mmSpecType">
              <el-select v-model="form.mmSpecType" filterable clearable  :placeholder="$t('btn.select')+'特殊采购类'">
                <el-option v-for="item in  options.sys_pur_spec " :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="散装物料" prop="mmBulkMat">
              <el-input clearable v-model="form.mmBulkMat" :placeholder="$t('btn.enter')+'散装物料'" />
            </el-form-item>
          </el-col>
    
          <el-col :lg="12">
            <el-form-item label="最小批量" prop="mmMoq">
              <el-input-number clearable v-model.number="form.mmMoq" :controls="true" controls-position="right" :placeholder="$t('btn.enter')+'最小批量'" />
            </el-form-item>
          </el-col>
    
          <el-col :lg="12">
            <el-form-item label="舍入值" prop="mmRoundingVal">
              <el-input-number clearable v-model.number="form.mmRoundingVal" :controls="true" controls-position="right" :placeholder="$t('btn.enter')+'舍入值'" />
            </el-form-item>
          </el-col>
    
          <el-col :lg="12">
            <el-form-item label="交货时间" prop="mmLeadTime">
              <el-input-number clearable v-model.number="form.mmLeadTime" :controls="true" controls-position="right" :placeholder="$t('btn.enter')+'交货时间'" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="生产天数" prop="mmProDays">
              <el-input-number clearable v-model.number="form.mmProDays" :controls="true" controls-position="right" :placeholder="$t('btn.enter')+'生产天数'" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="检验否" prop="mmisChecked">
              <el-select v-model="form.mmisChecked" filterable clearable  :placeholder="$t('btn.select')+'检验否'">
                <el-option v-for="item in  options.sys_flag_list " :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="利润中心" prop="mmProfitCenter">
              <el-select v-model="form.mmProfitCenter" filterable clearable  :placeholder="$t('btn.select')+'利润中心'">
          ...
          <el-col :lg="12">
            <el-form-item label="差异码" prop="mmDiffCode">
              <el-input clearable v-model="form.mmDiffCode" :placeholder="$t('btn.enter')+'差异码'" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="批次管理" prop="mmisLot">
              <el-input clearable v-model="form.mmisLot" :placeholder="$t('btn.enter')+'批次管理'" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="制造商物料" prop="mmMpn">
              <el-input clearable v-model="form.mmMpn" :placeholder="$t('btn.enter')+'制造商物料'" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="制造商" prop="mmMfrs">
              <el-input clearable v-model="form.mmMfrs" :placeholder="$t('btn.enter')+'制造商'" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="评估类" prop="mmValType">
              <el-select v-model="form.mmValType" filterable clearable  :placeholder="$t('btn.select')+'评估类'">
          ...
          <el-col :lg="12">
            <el-form-item label="移动平均价" prop="mmMovingAvg">
              <el-input-number clearable v-model.number="form.mmMovingAvg" :controls="true" controls-position="right" :placeholder="$t('btn.enter')+'移动平均价'" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="币种" prop="mmCcy">
              <el-select v-model="form.mmCcy" filterable clearable  :placeholder="$t('btn.select')+'币种'">
          ...
          <el-col :lg="12">
            <el-form-item label="价格控制" prop="mmPriceControl">
              <el-input clearable v-model="form.mmPriceControl" :placeholder="$t('btn.enter')+'价格控制'" />
            </el-form-item>
          </el-col>
    
          <el-col :lg="12">
            <el-form-item label="价格单位" prop="mmPriceUnit">
              <el-input-number clearable v-model.number="form.mmPriceUnit" :controls="true" controls-position="right" :placeholder="$t('btn.enter')+'价格单位'" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="采购仓库" prop="mmSLoc">
              <el-select v-model="form.mmSLoc" filterable clearable  :placeholder="$t('btn.select')+'采购仓库'">
          ...
          <el-col :lg="12">
            <el-form-item label="生产仓库" prop="mmESLoc">
              <el-select v-model="form.mmESLoc" filterable clearable  :placeholder="$t('btn.select')+'生产仓库'">
                <el-option v-for="item in  options.sys_sloc_list " :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="仓位" prop="mmLocPosn">
              <el-input clearable v-model="form.mmLocPosn" :placeholder="$t('btn.enter')+'仓位'" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="库存" prop="mmInventory">
              <el-input-number clearable v-model.number="form.mmInventory" :controls="true" controls-position="right" :placeholder="$t('btn.enter')+'库存'" />
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="EOL" prop="mmLocEol">
              <el-input clearable v-model="form.mmLocEol" :placeholder="$t('btn.enter')+'EOL'" />
            </el-form-item>
          </el-col>
          ...
          <el-col :lg="12">
            <el-form-item label="updateBy" prop="updateBy">
              <el-input clearable v-model="form.updateBy" :placeholder="$t('btn.enter')+'updateBy'"  :disabled="true"/>
            </el-form-item>
          </el-col>

          <el-col :lg="12">
            <el-form-item label="updateTime" prop="updateTime">
              <el-date-picker clearable v-model="form.updateTime" type="datetime" :teleported="false" :placeholder="$t('btn.dateselect')" :disabled="true"></el-date-picker>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
// 引入 mmmats操作方法
import { listMmMats, addMmMats, delMmMats, updateMmMats, getMmMats, 
 
 } 
from '@/api/material/mmmats.js'
//获取当前组件实例
...
//字段显示控制
const columns = ref([
  { visible: true, prop: 'mmId', label: 'ID' },
  { visible: true, prop: 'mmLastDate', label: '更新日期' },
  ...
  { visible: false, prop: 'mmPurType', label: '采购类型' },
...
function getList(){
  loading.value = true
  listMmMats(queryParams).then(res => {
    const { code, data } = res
    if (code == 200) {
...
function handleSelectionChange(selection) {
  ids.value = selection.map((item) => item.mmId);
...
const state = reactive({
  form: {},
  rules: {
    mmLastDate: [{ required: true, message: "更新日期"+proxy.$t('btn.empty'), trigger: "blur" }],
  ...
  options: {
    ...
    sys_sloc_list: [],
    // 生产仓库 选项列表 格式 eg:{ dictLabel: '标签', dictValue: '0'}
    sys_sloc_list: [],
  }
})
//将响应式对象转换成普通对象
...
function reset() {
  form.value = {
    ...
    mmBulkMat: undefined,
    mmMoq: undefined,
    mmRoundingVal: undefined,
    mmLeadTime: undefined,
    mmProDays: undefined,
    mmisChecked: undefined,
    mmProfitCenter: undefined,
    mmDiffCode: undefined,
    mmisLot: undefined,
    mmMpn: undefined,
    mmMfrs: undefined,
    mmValType: undefined,
    mmMovingAvg: undefined,
    mmCcy: undefined,
    mmPriceControl: undefined,
    mmPriceUnit: undefined,
    mmSLoc: undefined,
    mmESLoc: undefined,
    mmLocPosn: undefined,
    mmInventory: undefined,
    mmLocEol: undefined,
    reMark: undefined,
    createBy: undefined,
...
function handleUpdate(row) {
  reset()
  const id = row.mmId || ids.value
  getMmMats(id).then((res) => {
    const { code, data } = res
    if (code == 200) {
      open.value = true
...
function submitForm() {
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      if (form.value.mmId != undefined && opertype.value === 2) {
        updateMmMats(form.value).then((res) => {
...
function handleExport() {
  proxy
    ...
    .then(async () => {
      await proxy.downFile('/material/MmMats/export', { ...queryParams })
    })
}
